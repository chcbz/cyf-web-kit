...
- **fixture update fail-closed**：inventory/hash/preview/asset 四条路径都先执行代码常量 baseline 锁；重指 fixture 到当前 HEAD 或其他 commit 时 verify/update 均在写前失败。hash/asset provenance 使用 replacement-disabled blob-object 读取；隔离 clone 中真实建立 replace ref 的测试证明 verifier/update 仍以原 blob 为准，当前篡改被拒绝且 fixture 不变。asset 继续校验完整 27-file baseline public tree。共享 atomic writer 采用主错误 + cleanup errors 的 `AggregateError`；`atomicWriteUtf8Batch` 先完成全部 staging+fsync+verify，再以同目录 hard-link no-clobber 安装。trusted installed token 始终冻结自 staging descriptor，包含 dev+ino、mode、UTF-8 byte length 与 SHA-256，不会被后续 path 重采样覆盖。所有 target 校验均以 `O_NOFOLLOW` 打开同一 fd，执行 fstat-before/read/fstat-after，要求 regular file、读取期间 stat 稳定且 inode/mode/length/hash 精确匹配；rollback 仅删除完整匹配该 trusted token 的 target，同 inode 原地改写也视为 concurrent change 并保留。成功路径在 backup cleanup 后执行最后一次 descriptor-bound verification 作为函数内线性化边界；若 cleanup 窗口后发现变化，则从内存保存的原始 bytes 以 no-clobber 重建 recovery backup，保留并发 target 与明确列出路径的 intentional recovery artifact，再抛出 `AggregateError`。inventory 的 JSON+ledger 与 preview 的四 SVG 使用该批量事务；正常、原目标缺失、第二/第三目标失败、并发重建/替换、temp unlink 后同 inode 改写、backup cleanup 同 inode 改写、link/unlink/rollback cleanup 故障及成功线性化顺序均有注入测试。

...
describe('JuyiHall collaboration flow contract', () => {
  it('uses the stage header as the primary action surface without the duplicate dock', () => {
    ...
    expect(hallStageSource).to.include('hallPhysicalScene.interactiveZones')
    expect(hallStageSource).to.include('`room-${zone.key}`')
    expect(hallStageSource).to.include("emit('open-panel', zone.panel)")
  })

  it('distributes discussion entry points across courtyard bounty board and task cards', () => {
    expect(hallStageSource).to.include('openPublicDiscussion')
    expect(hallStageSource).to.include("zone.panel === 'chat'")
    expect(hallStageSource).to.include('room-${zone.key}')
  ...
  it('uses in-scene objects as click targets with integrated title hints', () => {
    ...
    expect(hallStageSource).to.include('hall-room-label')
    expect(hallStageSource).to.include('{{ zone.title }}')
    expect(hallStageSource).to.include('{{ zoneSubtitle(zone) }}')

...
assert.match(script, /const expectedMime = kind === 'text' && isDocumentTextPreview\(item\) \? TEXT_PREVIEW_MIME : item\.mimeType/)
assert.match(script, /blob\.type !== expectedMime/)
assert.ok(script.indexOf('blob.type !== expectedMime') < script.indexOf('URL.createObjectURL(blob)'), 'MIME must be checked before a Blob URL is created')
assert.match(script, /item\?\.mimeType === PDF_MIME/)
assert.match(outputs, /const pdfMimeTypes = new Set\(\['application\/pdf'\]\)/)
assert.match(outputs, /if \(pdfMimeTypes\.has\(item\.mimeType\)\) return 'pdf'/)

...
assert.match(script, /part\.legacyPdf && part\.blob\.size > MAX_PREVIEW_BYTES/)
assert.doesNotMatch(script, /part\.contentMimeType === 'image\/png'\) && part\.blob\.size > MAX_PREVIEW_BYTES/)
...
assert.match(source, /result\?\.partial === true/)
assert.match(source, /typeof result\?\.reason === 'string'/)
assert.match(source, /previewNote\.value = \[partialNote, documentNote\]\.filter\(Boolean\)\.join\(' '\)/)

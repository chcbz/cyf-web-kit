...
describe('C07A task workspace wiring', () => {
  ...
  it('uses atomic explicit JuyiHall action inputs, never brief/default or assignment completion, and disposes the real binding on unmount', () => {
    ...
    const identityGuardIndex = assignTaskSource.indexOf("if (!task?.id || !targetAgents.length || targetAgents.some(item => !hasExplicitAgentId(item))) return false")
    const eligibilityGuardIndex = assignTaskSource.indexOf('if (targetAgents.some(item => !canAssign(task, item))) return false')
    const clearActorIndex = assignTaskSource.indexOf('taskWorkspaceBinding.clearExplicitActor()')
    const runAssignmentIndex = assignTaskSource.indexOf('const assignmentSucceeded = await runAssignTask(task, agent)')
    const successGuardIndex = assignTaskSource.indexOf('if (!assignmentSucceeded) return false')
    const canonicalTaskIndex = assignTaskSource.indexOf('const canonicalTask = tasks.value.find(item => item.id === task.id) || task')
    const markAssignedIndex = assignTaskSource.indexOf('markTaskAssigned(canonicalTask, targetAgents)')
    expect(identityGuardIndex).to.be.greaterThan(-1)
    expect(eligibilityGuardIndex).to.be.greaterThan(identityGuardIndex)
    expect(clearActorIndex).to.be.greaterThan(eligibilityGuardIndex)
    expect(runAssignmentIndex).to.be.greaterThan(clearActorIndex)
    expect(successGuardIndex).to.be.greaterThan(runAssignmentIndex)
    expect(canonicalTaskIndex).to.be.greaterThan(successGuardIndex)
    expect(markAssignedIndex).to.be.greaterThan(canonicalTaskIndex)
    expect(assignTaskSource).to.include("canonicalTask.status === 'assigned' && canonicalTask.assignedAgentId === targetAgents[0].agentId")
    expect(assignTaskSource).not.to.include("task?.status === 'assigned'")
    ...
    const unmountStart = hallSource.lastIndexOf('onUnmounted(() => {')
    expect(unmountStart).to.be.greaterThan(-1)
    const unmountSource = hallSource.slice(unmountStart, hallSource.indexOf('\n})', unmountStart))
    expect(unmountSource).to.include('taskWorkspaceBinding.dispose()')
  ...
  it('preserves map/roster separation, no active route, and explicit assignment target handling', () => {
    ...
    expect(hallSource).to.include('const assignmentSucceeded = await runAssignTask(task, agent)')
    expect(hallTaskActionsSource).to.include('agentId: targetAgent.agentId')

    const canAssignStart = hallDataSource.indexOf('const canAssign =')
    const canAssignEnd = hallDataSource.indexOf('const taskStatusCount =', canAssignStart)
    expect(canAssignStart).to.be.greaterThan(-1)
    expect(canAssignEnd).to.be.greaterThan(canAssignStart)
    const canAssignSource = hallDataSource.slice(canAssignStart, canAssignEnd)
    const taskActionAssignSource = hallTaskActionsSource.slice(hallTaskActionsSource.indexOf('const assignTask'), hallTaskActionsSource.indexOf('const autoAssignTask'))
    expect(canAssignSource).to.include("typeof agent?.agentId !== 'string'")
    expect(canAssignSource).not.to.match(/agent\s*=\s*selectedAgent\.value/)
    expect(hallTaskActionsSource).to.include("const hasExplicitAgentId = item => typeof item?.agentId === 'string' && Boolean(item.agentId.trim())")
    expect(taskActionAssignSource).to.include('targetAgents.some(item => !hasExplicitAgentId(item))')
    expect(taskActionAssignSource).not.to.match(/agent\s*=\s*selectedAgent\.value/)
  })
})

...
describe('C07A task workspace wiring', () => {
  ...
  it('uses atomic explicit JuyiHall action inputs, never brief/default or assignment completion, and disposes the real binding on unmount', () => {
    ...
    const markAssignedIndex = assignTaskSource.indexOf('markTaskAssigned(task, targetAgents)')
    ...
    expect(markAssignedIndex).to.be.greaterThan(successGuardIndex)
  ...
  it('preserves map/roster separation, no active route, and explicit assignment target handling', () => {
    ...
    const canAssignSource = hallDataSource.slice(hallDataSource.indexOf('const canAssign'), hallDataSource.indexOf('const taskStatusCount'))
    ...
    expect(taskActionAssignSource).to.include("typeof item?.agentId === 'string'")

...
describe('live map preview orientation target transaction', () => {
  it('consumes a preview-queued target only after stable viewport commit and fences a reversed transition', async () => {
    ...
    try {
      ...
      await onboardingButton.trigger('click')
      expect(wrapper.emitted('open-onboarding')).to.deep.equal([[onboardingButton.element]])
      await wrapper.setProps({ interactionLocked: true }); await flush()
      expect(onboardingButton.attributes('disabled')).to.equal('')
      await onboardingButton.trigger('click')
      expect(wrapper.emitted('open-onboarding')).to.deep.equal([[onboardingButton.element]])
      await wrapper.setProps({ interactionLocked: false }); await flush()

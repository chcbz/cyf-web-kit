...
const LINK_FIELDS = new Set(['relationId', 'conversationId', 'fileId', 'version', 'role', 'state', 'relationRevision', 'createdAt'])
...
const randomKey = () => globalThis.crypto?.randomUUID?.() || `pws-conversation-link-${Date.now()}-${Math.random().toString(36).slice(2)}`
...
const validLink = (value, conversationId) => exactKeys(value, LINK_FIELDS) && value.conversationId === conversationId &&
...
const etagFor = link => validLink(link, link?.conversationId) ? `"${link.relationId}:${link.relationRevision}"` : ''
const validEtag = (value, link) => typeof value === 'string' && value === etagFor(link)
const validList = (value, conversationId) => exactKeys(value, LIST_FIELDS) && Array.isArray(value.items) &&
  value.items.length <= 50 && value.items.every(item => validLink(item, conversationId)) &&
  (value.nextCursor == null || CURSOR(value.nextCursor)) &&
  new Set(value.items.map(item => item.relationId)).size === value.items.length
const validDetach = (value, conversationId) => exactKeys(value, DETACH_FIELDS) && validLink(value.link, conversationId) &&
  value.link.state === 'DETACHED' && value.executionSnapshotsPreserved === true
const errorMessage = error => {
  if (error?.name === 'AbortError') return ''
  if ([401, 403, 404].includes(error?.status)) return '当前身份或议事资料已不可访问，请刷新后重新选择。'
  if (error?.code === 'IDEMPOTENCY_CONFLICT') return '本次关联请求与已有操作不一致，请刷新后确认关联记录。'
  if (error?.code === 'OUTPUT_LINK_MANAGED_BY_EXECUTION') return '议事成果由执行流程管理，不能在资料关联中修改。'
  if (error?.code === 'CONVERSATION_FILE_LINK_CHANGED' || error?.status === 412) return '关联已变化，请刷新后再解除。'
  ...
  if (error?.status === 503) return '议事资料关联暂不可用，请稍后刷新确认，不要重复提交。'
  return error?.message || '议事资料关联请求未完成，请刷新确认。'
}

/**
 * JWT-scoped conversation material directory. conversationId is only a resource selector: authorization
 * remains exclusively server-side, and this adapter never derives it from a route or Agent.
 */
export function usePersonalWorkspaceConversationLinks ({ api = createApi('/agent'), conversationId = null, identityEpoch = 0 } = {}) {
  const links = ref([])
  ...
  const contextKey = computed(() => ID(currentConversationId.value) ? `${currentEpoch.value}\u0000${currentConversationId.value}` : '')
  ...
  const reset = () => {
    generation += 1
    for (const controller of controllers) controller.abort(abortError('Conversation link context changed'))
    controllers.clear()
    links.value = []
  ...
  const request = async (options, snapshot = { generation, contextKey: contextKey.value }) => {
    ...
    try {
      ...
      if (!stillCurrent(snapshot, controller)) throw abortError('Conversation link context changed')
  ...
  const load = async ({ cursor = null, append = false } = {}) => {
    const selectedConversationId = currentConversationId.value
    const snapshot = { generation, contextKey: contextKey.value }
    if (!ID(selectedConversationId) || !snapshot.contextKey || loading.value || (cursor != null && !CURSOR(cursor))) return false
    loading.value = true
    if (!append) { listState.value = 'loading'; error.value = '' }
    try {
      const { data } = await request({
        url: `/conversations/${encodeURIComponent(selectedConversationId)}/file-links`, method: 'GET',
        params: cursor ? { cursor } : undefined
      }, snapshot)
      if (!validList(data, selectedConversationId)) throw new Error('议事资料关联目录返回格式无效，未展示可能不完整的数据。')
      if (append) {
  ...
  const attach = async ({ fileId, version, role } = {}) => {
    const selectedConversationId = currentConversationId.value
    if (!ID(selectedConversationId) || !ID(fileId) || !VERSION(version) || !['INPUT', 'REFERENCE'].includes(role)) {
    ...
    try {
      const { data, headers } = await request({
        url: `/conversations/${encodeURIComponent(selectedConversationId)}/file-links`, method: 'POST',
        data: { fileId, version, role }, headers: { 'Idempotency-Key': randomKey() }
      }, snapshot)
      if (!validLink(data, selectedConversationId) || !validEtag(header(headers, 'etag'), data)) {
        throw new Error('议事资料关联回执无效，尚不能确认保存。')
      }
      replace(data)
  ...
  const detach = async link => {
    const selectedConversationId = currentConversationId.value
    if (!ID(selectedConversationId) || !validLink(link, selectedConversationId) || link.state !== 'ACTIVE') {
    ...
    try {
      const { data, headers } = await request({
        url: `/conversations/${encodeURIComponent(selectedConversationId)}/file-links/${encodeURIComponent(link.relationId)}`,
        method: 'DELETE', headers: { 'Idempotency-Key': randomKey(), 'If-Match': ifMatch }
      }, snapshot)
      if (!validDetach(data, selectedConversationId) || data.link.relationId !== link.relationId || !validEtag(header(headers, 'etag'), data.link)) {
        throw new Error('解除议事资料关联回执无效，尚不能确认解除。')
      }
      replace(data.link)
      actionState.value = 'ready'
      return data
  ...
  return { links, nextCursor, listState, loading, actionState, error, currentConversationId, load, loadMore, attach, detach, etagFor, reset, dispose }
}

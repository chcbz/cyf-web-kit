<template>
  <section class="skill-market" aria-label="技能集市">
    ...
    <section v-if="market.unresolvedOperations.length" class="recovery-status" aria-live="polite">
      <strong>发现未解决购买操作</strong>
      <p>恢复会使用保存的商品版本、目标 Agent、版本、权限和原始请求，不会创建新报价或订单。</p>
      <div v-for="operation in market.unresolvedOperations" :key="operation.intentFingerprint" class="recovery-item">
        <span>{{ operation.productVersionId }} / {{ operation.targetAgentId }} / v{{ operation.expectedAgentVersion }}</span>
        <var-button text type="primary" size="small" @click="resumeOperation(operation)">{{ operation.phase === 'ORDER' && operation.orderId ? '检查订单' : '恢复操作' }}</var-button>
      </div>
    </section>
...
<style scoped>
.skill-market { display: grid; gap: 16px; padding: 18px; color: #31445c; } .market-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; } h2, h3, p { margin: 0; } h2 { color: #263d58; } .market-header p { margin-top: 5px; color: #748197; } .disabled-notice, .error { padding: 10px 12px; border-radius: 8px; } .disabled-notice { background: #fff7e8; color: #93611c; } .error { background: #fff0f0; color: #a33f3f; } .product-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(235px, 1fr)); gap: 12px; } .facts, .order-status, .recovery-status { display: grid; gap: 6px; padding: 14px; border: 1px solid #d9e3ee; border-radius: 10px; background: #f9fbfd; } .facts p, .order-status span, .recovery-status p { color: #5d6c80; font-size: 13px; } .recovery-item { display: flex; align-items: center; justify-content: space-between; gap: 12px; color: #5d6c80; font-size: 13px; } .order-status-heading { display: flex; align-items: center; justify-content: space-between; gap: 12px; } @media (max-width: 640px) { .market-header { flex-direction: column; } }
</style>

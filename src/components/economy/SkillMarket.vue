...
import { orderStatusLabel, useSkillMarket } from '@/composables/useSkillMarket.js'
...
const installedText = computed(() => props.installedSkills.map(item => typeof item === 'string' ? item : item.skillKey || item.name || item.version).filter(Boolean).join('、') || '未报告已安装技能')
...
const completePurchase = async () => {
  ...
  if (order) purchaseVisible.value = false
}

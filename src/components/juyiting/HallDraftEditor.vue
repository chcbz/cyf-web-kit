  ...
  <section class="hall-draft-editor" aria-labelledby="hall-draft-editor-title">
    <header>
      <div>
        ...
        <p>先保存草稿、固定资料版本和受托好汉；当前合同不提交、不执行，也不会生成新悬赏。</p>
      </div>
    ...
    <form @submit.prevent="persist">
      ...
      <p v-else-if="drafts.draft.value" class="draft-saved" role="status">草稿 {{ drafts.draft.value.draftId }} 已保存为 r{{ drafts.draft.value.revision }}；尚未提交或执行。</p>
      <footer><button type="submit" :disabled="busy">{{ drafts.draft.value ? '保存修改' : '确认保存草稿' }}</button><button
        v-if="drafts.draft.value"
        type="button"
        :disabled="busy"
        @click="discard"
      >放弃草稿</button></footer>
...
const busy = computed(() => ['creating', 'saving', 'loading', 'discarding'].includes(drafts.state.value))

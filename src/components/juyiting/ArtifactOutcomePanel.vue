<template>
  <section class="artifact-outcome-panel" aria-labelledby="artifact-outcome-heading">
    ...
    <section aria-labelledby="accepted-outcomes-heading">
      <div class="artifact-outcome-heading"><h4 id="accepted-outcomes-heading">权威已接受成果</h4><button type="button" :disabled="outcomes.acceptedState === 'loading'" @click="outcomes.refreshAccepted">刷新权威结果</button></div>
      <p class="artifact-outcome-status" :class="`is-${outcomes.acceptedState}`" role="status" aria-live="polite">{{ outcomes.acceptedMessage || '尚未读取权威接受结果。' }}</p>
      ...
      <ul v-if="outcomes.accepted.length" class="artifact-outcome-list" aria-label="服务端确认的已接受成果">
        <li v-for="row in outcomes.accepted" :key="`${row.artifactId}:${row.artifactVersion}`">
          <label><input type="checkbox" :checked="outcomes.supersededSelections.includes(`${row.artifactId}\u0000${row.artifactVersion}`)" @change="outcomes.toggleSuperseded(row)" />取代</label>
    ...
    <form class="artifact-outcome-form" @submit.prevent="outcomes.accept">
      <h4>明确确认一次裁决</h4>
      <label>当前工作台可见成果
        <select :value="outcomes.acceptedSelection" required @change="selectArtifact">
      ...
      <label>该成果当前裁决版本（精确 CAS 值）
        <input v-model.trim="outcomes.expectedOutcomeVersion" inputmode="numeric" pattern="0|[1-9][0-9]*" placeholder="例如 0" required />
      </label>
      <p class="artifact-outcome-note">工作台快照没有历史裁决版本。请只填已明确取得的精确 CAS 值；无法取得时，此接口契约不能安全重新接受。</p>
      <label class="artifact-outcome-confirm"><input v-model="outcomes.confirmed" type="checkbox" />我确认接受所选成果，并取代勾选的服务端已接受成果。</label>
      <p v-if="outcomes.submitMessage" class="artifact-outcome-status" :class="`is-${outcomes.submitState}`" role="status" aria-live="polite">{{ outcomes.submitMessage }}</p>

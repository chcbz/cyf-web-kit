<template>
  <section class="bounty-discussion-panel discussion-panel">
    ...
    <ChatPanel
      ...
      empty-text="这张悬赏还没有议事记录，先同步风险、分工和下一步。"
      placeholder="围绕当前悬赏发起议事"
...
const bountySubtitle = computed(() => {
  const taskName = props.selectedTask?.title || props.selectedTask?.id || '当前悬赏'
  const countText = props.agents.length ? ` / ${props.agents.length} 位参与人` : ''

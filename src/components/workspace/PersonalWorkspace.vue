<template>
  <main class="personal-workspace">
    ...
    <section class="workspace-card" aria-labelledby="workspace-upload-title">
      ...
      <p class="workspace-note">支持 PNG、JPEG、纯文本、PDF、DOCX、XLSX、PPTX。Office/PDF 当前可保存和下载，预览及 Agent 编辑将在后续版本开放。</p>
    ...
    <section class="workspace-card" aria-labelledby="workspace-files-title">
      ...
      <div v-else class="file-list">
        <button v-for="file in workspace.items.value" :key="file.fileId" type="button" class="file-row" :class="{ selected: selectedId === file.fileId }" @click="select(file.fileId)">
      ...
      <button v-if="workspace.nextCursor.value" type="button" class="load-more" :disabled="workspace.loading.value" @click="loadMore">加载更多</button>
    </section>
    ...
    <section v-if="workspace.detail.value" class="workspace-card detail-card" aria-labelledby="workspace-detail-title">
      ...
      <section class="versions" aria-labelledby="workspace-version-title">
        ...
        <div v-for="version in workspace.detail.value.versions" :key="version.version" class="version-row" :class="{ active: selectedVersion === version.version }">
      ...
      <div class="danger-zone">
        <template v-if="workspace.detail.value.file.state === 'ACTIVE'"><p>移入回收站后不会永久删除，当前版本和历史版本仍可恢复。</p><button type="button" class="danger" :disabled="workspace.actionState.value === 'saving'" @click="trash">移入回收站</button></template>
...
const workspace = usePersonalWorkspace({ identityEpoch: computed(() => apiStore.authorizationGeneration) })
const query = ref('')
...
.workspace-header { display: flex; justify-content: space-between; gap: 20px; padding: 20px; }.workspace-header h1,.workspace-header p,.workspace-card h2,.workspace-card h3 { margin-top: 0; }.workspace-header h1 { margin-bottom: 6px; }.workspace-header p { margin-bottom: 0; color: #64748b; }.workspace-eyebrow { color: #4f46e5 !important; font-weight: 700; }.workspace-header button,.workspace-card button { min-height: 38px; padding: 0 12px; border: 1px solid #6366f1; border-radius: 8px; background: #fff; color: #4338ca; cursor: pointer; }.workspace-header button:hover,.workspace-card button:hover { background: #eef2ff; }.workspace-card button:disabled { cursor: not-allowed; opacity: .6; }.workspace-card { padding: 18px; }.workspace-note,.section-heading p { color: #64748b; font-size: 14px; }.workspace-card label { display: grid; gap: 6px; margin-top: 12px; color: #475569; font-size: 14px; }.workspace-card input,.workspace-card select { min-height: 38px; padding: 0 10px; border: 1px solid #cbd5e1; border-radius: 8px; font: inherit; }.file-picker { display: inline-flex !important; width: fit-content; position: relative; overflow: hidden; padding: 9px 12px; border-radius: 8px; background: #eef2ff; color: #3730a3 !important; cursor: pointer; }.file-picker input { position: absolute; inset: 0; opacity: 0; cursor: pointer; }.selected-file,.workspace-receipt { color: #475569; font-size: 14px; }.actions,.filter-actions,.version-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 14px; }.section-heading { display: flex; justify-content: space-between; gap: 16px; }.section-heading h2 { margin-bottom: 6px; }.filters { display: grid; grid-template-columns: minmax(0,1fr) 180px auto; align-items: end; gap: 12px; padding: 12px 0; border-top: 1px solid #e2e8f0; }.filter-actions { margin: 0; }.filter-actions .active { background: #4f46e5; color: #fff; }.file-list { display: grid; gap: 8px; }.file-row { display: grid; grid-template-columns: 32px minmax(0,1fr) auto; align-items: center; width: 100%; text-align: left; border-color: #e2e8f0 !important; color: #1f2937 !important; }.file-row.selected { border-color: #4f46e5 !important; background: #eef2ff !important; }.file-summary { display: grid; min-width: 0; gap: 3px; }.file-summary strong,.file-summary small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.file-summary small,.file-state { color: #64748b; }.file-state { font-size: 12px; }.workspace-empty { padding: 24px 0; color: #64748b; }.load-more { margin-top: 12px; }.file-details { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 10px; }.file-details div { padding: 10px; border-radius: 8px; background: #f8fafc; }.file-details dt { color: #64748b; font-size: 12px; }.file-details dd { margin: 4px 0 0; word-break: break-word; }.inline-form,.version-upload { display: flex; flex-wrap: wrap; align-items: end; gap: 10px; margin-top: 16px; padding-top: 16px; border-top: 1px solid #e2e8f0; }.inline-form label { flex: 1; min-width: 220px; margin: 0; }.version-upload .file-picker { margin: 0; }.versions { margin-top: 20px; }.version-row { display: grid; grid-template-columns: auto minmax(0,1fr) auto auto; align-items: center; gap: 10px; padding: 10px 0; border-bottom: 1px solid #e2e8f0; }.version-row.active { background: #f8fafc; }.version-row > span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.version-row small { color: #64748b; }.version-actions { justify-content: flex-end; margin: 0; }.preview { margin-top: 16px; overflow: auto; border-radius: 8px; background: #f8fafc; }.preview pre { margin: 0; padding: 12px; white-space: pre-wrap; overflow-wrap: anywhere; }.preview img { display: block; max-width: 100%; max-height: 460px; margin: auto; object-fit: contain; }.preview p { padding: 12px; color: #64748b; }.danger-zone { display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 12px; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e2e8f0; }.danger-zone p { margin: 0; color: #64748b; }.danger-zone .danger { border-color: #dc2626; color: #b91c1c; }.workspace-error { max-width: 980px; margin: 0 auto 16px; padding: 12px; border-radius: 8px; background: #fef2f2; color: #b91c1c; }.workspace-receipt { max-width: 980px; margin: 0 auto; }

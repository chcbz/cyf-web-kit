...
function main(){
 ...
 const probeBad=shots.filter(s=>s.probeMobility!=='synthetic-visual-only'||s.navValidation?.reachability?.source!=='production-graph-pathfinder'||s.navValidation?.reachability?.colliderWidth!==42||!['found','blocked'].includes(s.navValidation?.reachability?.status)||(s.probeKind==='target-specific'&&((s.visualExerciseContract==='target-each-shot'||(['ownership-transition','composite-transition'].includes(s.visualExerciseContract)&&s.relation==='behind'))&&(s.runtimeFacts?.pixelOverlap?.opaqueIntersectionPixels<=0||s.runtimeFacts?.pixelOverlap?.visibleOcclusionPixels<=0))))
 check('synthetic visual probes retain production navigation diagnostics and target-specific pixel exercise constraints',probeBad.length===0,probeBad.slice(0,12).map(s=>s.id).join(','))

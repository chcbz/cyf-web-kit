...
Per-shot: determine agent depth vs each fragment, render agent, then re-render
any fragments that should occlude the agent on top.
"""
import os
...
class OfflineRenderer:
    ...
    def __init__(self, public_dir):
        ...
        self._full_composite = None  # base + all fragments + all props + fg + lt
        self._frag_imgs = {}  # pre-loaded fragment pixel data

    def _build_full_composite(self, fragments, props_list):
        """Pre-render the full map: base + all fragments in sort order + props + fg + lighting."""
        if self._full_composite is not None:
            return
        import time
        t0 = time.time()

        # Sort fragments + props by base order (no agent)
    ...
    def render_shot(self, shot, fragments, props_list):
        ...
        # Copy viewport from full composite
        ...
        # Determine agent's depth
        agent_depth = depths.get(agent['stableId'], 0)
        agent_sid = agent['stableId']

        # Find fragments that are BEHIND the agent (higher depth = rendered later = in front)
        # Objects with depth > agent_depth appear IN FRONT of agent
        # Objects with depth < agent_depth appear BEHIND agent
        # The full composite has everything. We need to:
        # 1. Re-render fragments that should be IN FRONT of agent (depth > agent_depth) ON TOP
        #    (because they're currently under where the agent will be drawn)
        # 2. Render agent
        # 3. All fragments behind agent stay as-is in the composite
        ...
        for obj in sorted_objects:
            ...
            d = depths.get(sid, 0)
            if d > agent_depth:
                covering_frags.append(obj)

        # Render agent
        self._render_agent_vp(canvas, agent, wx, wy)

        # Re-render covering fragments on top of agent (within viewport)
        for obj in covering_frags:
            if not self._obj_overlaps(obj, (wx, wy, cw, ch)):
                continue
            if obj.get('kind') == 'prop':
        ...
        # Runtime facts
        target_sid = shot['targetStableId']
    ...
    def render_shot_small(self, shot, fragments, props_list, crop_w=400, crop_h=300):
        """Render a small crop centered on agent-target midpoint. Faster + smaller output."""
        from .world_model import (
            build_agent_scene_object, compute_unified_order,
            compute_world_sort_key, PERSONA_SPRITE, PERSONAS,
        )
        ...
        # Crop center: midpoint between agent and target anchor
        ...
        # Determine agent depth vs target
        agent_sid = agent['stableId']
        agent_depth = depths.get(agent_sid, 0)

        # Re-render fragments that should occlude the agent
        covering = []
        vp_rect = (wx, wy, cw, ch)
        for obj in sorted_objects:
            sid = obj.get('stableId', '')
            if sid == agent_sid: continue
            d = depths.get(sid, 0)
            if d > agent_depth and self._obj_overlaps(obj, vp_rect):
                covering.append(obj)

        # Render agent
        self._render_agent_vp(canvas, agent, wx, wy)

        # Re-render covering fragments on top
        for obj in covering:
            if obj.get('kind') == 'prop':
                self._render_prop_vp(canvas, obj, wx, wy)
            else:
